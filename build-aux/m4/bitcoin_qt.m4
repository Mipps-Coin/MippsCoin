dnl Copyright (c) 2013-2016 The Mippscoin Core developers
dnl Distributed under the MIT software license, see the accompanying
dnl file COPYING or http://www.opensource.org/licenses/mit-license.php.

dnl Helper for cases where a qt dependency is not met.
dnl Output: If qt version is auto, set mippscoin_enable_qt to false. Else, exit.
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "$mippscoin_qt_want_version" = "auto" && test "$mippscoin_qt_force" != "yes"; then
    if test "$mippscoin_enable_qt" != "no"; then
      AC_MSG_WARN([$1; mippscoin-qt frontend will not be built])
    fi
    mippscoin_enable_qt=no
    mippscoin_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])
...
AC_DEFUN([BITCOIN_QT_CHECK],[
  if test "$mippscoin_enable_qt" != "no" && test "$mippscoin_qt_want_version" != "no"; then
    true
    $1
  else
    true
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build mippscoin-qt GUI (default=auto)])],
    [
     mippscoin_qt_want_version=$withval
     if test "$mippscoin_qt_want_version" = "yes"; then
       mippscoin_qt_force=yes
       mippscoin_qt_want_version=auto
     fi
    ],
    [mippscoin_qt_want_version=auto])
  ...
  AS_IF([test "$with_gui" = "qt5_debug"],
        [AS_CASE([$host],
                 ...
                 [qt_lib_suffix= ]); mippscoin_qt_want_version=qt5],
        [qt_lib_suffix= ])
...
dnl Outputs: mippscoin_enable_qt, mippscoin_enable_qt_dbus, mippscoin_enable_qt_test
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  if test "$mippscoin_cv_static_qt" = "yes"; then
  ...
  BITCOIN_QT_CHECK([
    mippscoin_enable_qt=yes
    mippscoin_enable_qt_test=yes
    if test "$have_qt_test" = "no"; then
      mippscoin_enable_qt_test=no
    fi
    mippscoin_enable_qt_dbus=no
    if test "$use_dbus" != "no" && test "$have_qt_dbus" = "yes"; then
      mippscoin_enable_qt_dbus=yes
    fi
  ...
  if test $mippscoin_enable_qt = "yes"; then
    AC_MSG_RESULT([$mippscoin_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$mippscoin_enable_qt])
  fi
...
dnl Output: mippscoin_cv_static_qt=yes|no
AC_DEFUN([_BITCOIN_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, mippscoin_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
        #ifndef QT_VERSION
        #  include <QtCore/qglobal.h>
      ...
      [mippscoin_cv_static_qt=yes],
      [mippscoin_cv_static_qt=no])
    ])
])
