...
def parse_config_into_btc_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Mippscoin Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/mippscoin_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/mippscoin_config.h'), "w", encoding="utf8") as btc_config:
...
def main():
    parser = argparse.ArgumentParser(description='Mippscoin-core msbuild configuration initialiser.')
    ...
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/mippscoin_config.h'), os.path.join(SOURCE_DIR, 'config/mippscoin-config.h'))
