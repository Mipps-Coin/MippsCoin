...
/** Call node.updateRwSetting() with Mippscoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Mippscoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/mippscoin/mippscoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because mippscoin 22.x will not try to read these.
...
//! Convert enabled/size values to mippscoin -prune setting.
...
//! Get pruning enabled value to show in GUI from mippscoin -prune setting.
...
//! Get pruning size value to show in GUI from mippscoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayMippscoinUnit")) {
        settings.setValue("DisplayMippscoinUnit", QVariant::fromValue(MippscoinUnit::BTC));
    }
    QVariant unit = settings.value("DisplayMippscoinUnit");
    if (unit.canConvert<MippscoinUnit>()) {
        m_display_mippscoin_unit = unit.value<MippscoinUnit>();
    } else {
        m_display_mippscoin_unit = MippscoinUnit::BTC;
        settings.setValue("DisplayMippscoinUnit", QVariant::fromValue(m_display_mippscoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if mippscoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_mippscoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<MippscoinUnit>() == m_display_mippscoin_unit) return;
    m_display_mippscoin_unit = new_unit.value<MippscoinUnit>();
    QSettings settings;
    settings.setValue("DisplayMippscoinUnit", QVariant::fromValue(m_display_mippscoin_unit));
    Q_EMIT displayUnitChanged(m_display_mippscoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/mippscoin/mippscoin/pull/8273
    ...
    // (https://github.com/mippscoin-core/gui/issues/567).
