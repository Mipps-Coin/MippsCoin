// Copyright (c) 2011-2022 The Mippscoin Core developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <qt/mippscoingui.h>
...
const std::string MippscoinGUI::DEFAULT_UIPLATFORM =
...
MippscoinGUI::MippscoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &MippscoinGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &MippscoinGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &MippscoinGUI::showModalOverlay);
...
void MippscoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Mippscoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and mippscoin: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &MippscoinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &MippscoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &MippscoinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Mippscoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Mippscoin addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Mippscoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Mippscoin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a mippscoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Mippscoin command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &MippscoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &MippscoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &MippscoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &MippscoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &MippscoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &MippscoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &MippscoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &MippscoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &MippscoinGUI::createWallet);
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_migrate_wallet_action, &QAction::triggered, [this] {
            auto activity = new MigrateWalletActivity(m_wallet_controller, this);
            connect(activity, &MigrateWalletActivity::migrated, this, &MippscoinGUI::setCurrentWallet);
            activity->migrate(walletFrame->currentWalletModel());
        });
        connect(m_mask_values_action, &QAction::toggled, this, &MippscoinGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &MippscoinGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &MippscoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &MippscoinGUI::showDebugWindow);
}

void MippscoinGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void MippscoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &MippscoinGUI::setCurrentWalletBySelectorIndex);
...
void MippscoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &MippscoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &MippscoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &MippscoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &MippscoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void MippscoinGUI::enableHistoryAction(bool privacy)
{
    historyAction->setEnabled(!privacy);
    if (historyAction->isChecked()) gotoOverviewPage();
}

void MippscoinGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &MippscoinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &MippscoinGUI::removeWallet);
...
WalletController* MippscoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void MippscoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &MippscoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &MippscoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &MippscoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &MippscoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &MippscoinGUI::incomingTransaction);
    connect(this, &MippscoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void MippscoinGUI::removeWallet(WalletModel* walletModel)
{
...
void MippscoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void MippscoinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void MippscoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void MippscoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void MippscoinGUI::createTrayIcon()
{
...
void MippscoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &MippscoinGUI::toggleHidden);
...
void MippscoinGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void MippscoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void MippscoinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void MippscoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void MippscoinGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void MippscoinGUI::openClicked()
{
...
void MippscoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void MippscoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void MippscoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void MippscoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void MippscoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void MippscoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void MippscoinGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void MippscoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Mippscoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void MippscoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void MippscoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void MippscoinGUI::updateHeadersSyncProgressLabel()
{
...
void MippscoinGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void MippscoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &MippscoinGUI::quitRequested);
...
void MippscoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void MippscoinGUI::createWallet()
{
#ifdef ENABLE_WALLET
#ifndef USE_SQLITE
    ...
    connect(activity, &CreateWalletActivity::created, this, &MippscoinGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void MippscoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void MippscoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &MippscoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &MippscoinGUI::show);
                e->ignore();
            }
        }
    }
...
void MippscoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void MippscoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void MippscoinGUI::incomingTransaction(const QString& date, MippscoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(MippscoinUnits::formatWithUnit(unit, amount, true));
...
void MippscoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void MippscoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool MippscoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool MippscoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void MippscoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void MippscoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void MippscoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void MippscoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void MippscoinGUI::updateWindowTitle()
{
...
void MippscoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void MippscoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void MippscoinGUI::detectShutdown()
{
...
void MippscoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void MippscoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(MippscoinGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = MippscoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void MippscoinGUI::subscribeToCoreSignals()
{
...
void MippscoinGUI::unsubscribeFromCoreSignals()
{
...
bool MippscoinGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<MippscoinUnit> units = MippscoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const MippscoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, MippscoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const MippscoinUnit u : MippscoinUnits::availableUnits()) {
        menu->addAction(MippscoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(MippscoinUnit newUnits)
{
    setText(MippscoinUnits::longName(newUnits));
}
