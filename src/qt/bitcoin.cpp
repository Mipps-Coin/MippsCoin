...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<MippscoinUnit>("MippscoinUnit");
#else
    qRegisterMetaType<MippscoinUnit>("MippscoinUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. mippscoin_de.qm (shortcut "de" needs to be defined in mippscoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. mippscoin_de_DE.qm (shortcut "de_DE" needs to be defined in mippscoin.qrc)
...
static const char* qt_argv = "mippscoin-qt";

MippscoinApplication::MippscoinApplication()
...
void MippscoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the MippscoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", MippscoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void MippscoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool MippscoinApplication::createOptionsModel(bool resetSettings)
{
...
void MippscoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new MippscoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &MippscoinGUI::quitRequested, this, &MippscoinApplication::requestShutdown);
...
void MippscoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void MippscoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool MippscoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void MippscoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &MippscoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &MippscoinApplication::handleRunawayException);
    connect(this, &MippscoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &MippscoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void MippscoinApplication::parameterSetup()
{
...
void MippscoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void MippscoinApplication::requestInitialize()
{
...
void MippscoinApplication::requestShutdown()
{
...
void MippscoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // mippscoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &MippscoinGUI::handlePaymentRequest);
            connect(window, &MippscoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void MippscoinApplication::handleRunawayException(const QString &message)
{
...
void MippscoinApplication::handleNonFatalException(const QString& message)
{
...
WId MippscoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool MippscoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", MippscoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(mippscoin);
    Q_INIT_RESOURCE(mippscoin_locale);
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see mippscoin-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, PACKAGE_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see mippscoin-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse mippscoin.conf, determine network, switch to network specific
    ...
    // mippscoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
