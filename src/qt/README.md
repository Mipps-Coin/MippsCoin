This directory contains the source code for the Mippscoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Mippscoin UI.
...
- ClientModel is used by the main application `mippscoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a mippscoin:... link from a browser).
...
* UI elements like MippscoinAmountField, which inherit from QWidget.
* `mippscoinstrings.cpp`: automatically generated
* `mippscoinunits.(h/cpp)`: BTC / mBTC / etc. handling
* `callback.h`
...
**Note:** Do not change `local/mippscoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `mippscoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `mippscoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `mippscoin-qt`
9. You're all set! Start developing, building, and debugging the Mippscoin Core GUI
