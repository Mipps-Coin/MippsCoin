...
const char * const BITCOIN_CONF_FILENAME = "mippscoin.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //mippscoin-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Mippscoin
    // macOS: ~/Library/Application Support/Mippscoin
    // Unix-like: ~/.mippscoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Mippscoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Mippscoin";
#else
    // Unix-like
    return pathRet / ".mippscoin";
#endif
#endif
}
