# User-space, Statically Defined Tracing (USDT) for Mippscoin Core

Mippscoin Core includes statically defined tracepoints to allow for more
...
**Note**: Mippscoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Mippscoin Core
...
Multiple tools can list the available tracepoints in a `mippscoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Mippscoin Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/mippscoind
stap net        outbound_message 0x0000000000107c05 /src/mippscoind
stap validation block_connected  0x00000000002fb10c /src/mippscoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Mippscoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/mippscoind | grep NT_STAPSDT -A 4 -B 2
