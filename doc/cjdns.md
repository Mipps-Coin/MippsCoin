# CJDNS support in Mippscoin Core

It is possible to run Mippscoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Mippscoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Mippscoin network and individual nodes.
...
before it will work with your Mippscoin Core node. This is described in steps
...
## Run Mippscoin Core with CJDNS

Once you are connected to the CJDNS network, the following Mippscoin Core
...
When enabled, this option tells Mippscoin Core that it is running in an
...
IPv6 local network. This helps Mippscoin Core perform better address management:
...
CJDNS support was added to Mippscoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `mippscoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/mippscoin/mippscoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Mippscoin Core

There are several ways to see your CJDNS address in Mippscoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `mippscoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `mippscoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `mippscoin-cli help getnodeaddresses` for details.
