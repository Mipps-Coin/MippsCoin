#!/usr/bin/env python3
# Copyright (c) 2020-2022 The Mippscoin Core developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing mippscoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='mippscoin-cli', help='mippscoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Mippscoin address to which the faucet should send')
...
parser.add_argument('mippscoin_cli_args', nargs='*', help='Arguments to pass on to mippscoin-cli (default: -signet)')

args = parser.parse_args()

if args.mippscoin_cli_args == []:
    args.mippscoin_cli_args = ['-signet']


def mippscoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.mippscoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = mippscoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = mippscoin_cli(['getnewaddress', 'faucet', 'bech32'])
