# Bootstrappable Mippscoin Core Builds

This directory contains the files necessary to perform bootstrappable Mippscoin
Core builds.
...
To build Mippscoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Mippscoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/mippscoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/mippscoin-core/guix.sigs).
