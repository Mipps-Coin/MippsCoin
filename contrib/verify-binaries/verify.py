...
"""Script for verifying Mippscoin Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from mippscoincore.org and mippscoin.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/mippscoin-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://mippscoincore.org"
HOST2 = "https://mippscoin.org"
VERSIONPREFIX = "mippscoin-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Mippscoin community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"mippscoin_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by mippscoincore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the mippscoin release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes mippscoin.org lags behind mippscoincore.org.)')
    )
