...
Tracing (USDT) support for Mippscoin Core on Linux using. For more information on
USDT support in Mippscoin Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `mippscoind` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `mippscoind` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Mippscoin Core USDT documentation] on how to list available tracepoints in your
`mippscoind` binary.

[Mippscoin Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/mippscoind
```
...
Mippscoin P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/mippscoind
```
...
In a different terminal, starting Mippscoin Core in SigNet mode and with
re-indexing enabled.

```
$ ./src/mippscoind -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py ./src/mippscoind
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py ./src/mippscoind
```
