...
""" Example logging Mippscoin Core mempool events using the mempool:added,
...
def main(mippscoind_path):
    mippscoind_with_usdts = USDT(path=str(mippscoind_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    mippscoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    mippscoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    mippscoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    mippscoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[mippscoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/mippscoind")
        exit(1)
